...
export default function CaseVisitsPanel({ caseItem }: { caseItem: Case }) {
  ...
  const save = async () => { try { editing ? await updateCaseVisit(caseItem.id, editing.id, form) : await createCaseVisit(caseItem.id, form); setOpen(false); await load(); }
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between"><h3 className="font-semibold">Case Visits</h3><button onClick={startAdd} className="rounded-lg bg-orange-600 px-3 py-2 text-sm text-white">Add Visit</button></div>
    ...
    <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b text-left">{["Visit Type","Address","District","City","Executive","Status","Receive Date","Closed Date","TAT","Remarks","Action"].map(x=><th key={x} className="p-2">{x}</th>)}</tr></thead>
      <tbody>{visits.map(v=><tr key={v.id} className="border-b"><td className="p-2">{v.visit_type}</td><td className="p-2">{v.address||"-"}</td><td className="p-2">{v.district||"-"}</td><td className="p-2">{v.city||"-"}</td><td className="p-2">{v.executive||"-"}</td><td className="p-2"><StatusBadge status={v.status}/></td><td className="p-2">{v.receive_date||"-"}</td><td className="p-2">{v.closed_date||"-"}</td><td className="p-2">{v.tat_days == null ? "-" : `${v.tat_days} days`}</td><td className="p-2">{v.remarks||"-"}</td><td className="p-2 whitespace-nowrap"><button onClick={()=>startEdit(v)} className="mr-2 text-orange-700">View / Edit</button><button onClick={()=>void remove(v)} className="text-red-600">Delete</button></td></tr>)}</tbody></table></div>
    {open && <div className="rounded-xl border bg-slate-50 p-4 dark:bg-slate-800"><div className="grid gap-3 md:grid-cols-2">
      ...
      {(["address","city","landmark","executive","receive_date","remarks","negative_reason","follow_up_note"] as const).map(key=><label key={key} className={key==="address"||key==="remarks"?"md:col-span-2":""}>{key.replaceAll("_"," ")}<input type={key==="receive_date"?"date":"text"} value={form[key]??""} onChange={e=>set(key,e.target.value)} className="block w-full rounded border p-2"/></label>)}
